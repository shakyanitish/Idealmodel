...
if (!empty($popRec)) {
    ...
    // Updated to use Bootstrap 4 style (data-dismiss and close class)
    $restst .= ' 
    <div class="modal fade" id="modal-popup-image" tabindex="-1" role="dialog" aria-hidden="true">
    ...
    foreach ($popRec as $popr) {
        if (($popr->image) != "a:0:{}") {
            $images = unserialize($popr->image);
            foreach ($images as $imgname) {
                $file_path = SITE_ROOT . 'images/popup/' . $imgname;
                if (file_exists($file_path)) {
                    $imglink = IMAGE_PATH . 'popup/' . $imgname;
                } else {
                    $imglink = BASE_URL . 'template/cms/images/welcome.jpg';
                }
                $active = ($count == 1) ? 'active' : '';
                ...
                $restst .= '   
                <div class="carousel-item ' . $active . '">
                    <a href="' . $linkhref . '" ' . $target . '><img src="' . $imglink . '" alt="' . $popr->title . '" class="d-block w-100"></a>
                </div>
                ';
                $active = ''; 
                $count++;
